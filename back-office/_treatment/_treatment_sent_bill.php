...
// $emailDestinataire = ($factureData['email']);
// $nameDestinataire = ($factureData['name']);
...
if (isset($id_bill) && $id_bill == $idBill) {
    // Obtenez le contenu du PDF depuis la session
    $pdfPath = $_SESSION['pdf_content_' . $id_bill];
    // var_dump($_POST);
    // exit;
...
$response = $mj->post(Resources::$Email, ['body' => $body]);
    ...
    if ($response->success()) {
        $_SESSION['notif'] = ['type' => 'success', 'message' => 'Factures envoyé avec succès!'];
    } else {
        $_SESSION['notif'] = ['type' => 'error', 'message' => 'Erreur lors de l\'envoi de la facture : ' . $response->getStatus() . ' - ' . json_encode($response->getData())];
    }

    // //   Requête SQL pour mettre à jour le devis
    $sqlEstimate = $dtLb->prepare("UPDATE factures SET is_sent = 1 WHERE id_bill = :id_bill");
    $sqlEstimate->execute(['id_bill' => $id_bill]);
